...
class DrProfile():
    def __init__(
        ...
        sr_model_id=None, sr_model_dscp=None, dr_model_id=None, dr_model_dscp=None, load_path='.', save_path='.',
        dirc_mets='dl_lost',
    ...
    def setup_profile_by_packet(self, df, ho_df):
        scope, mets = self.scope_models, self.mets
        this_df = df.copy()
        ...
        for i, row in ho_df.iterrows():
            ...
            if not stable_df.empty:
                ...
                for ts in unique_timestamps:
                    ...
                    if not tmp_df[(tmp_df['Timestamp'] >= stable_interval.lower) & (tmp_df['Timestamp'] < stable_interval.upper)].empty:
                        df.loc[(df['Timestamp'] >= stable_interval.lower) & (df['Timestamp'] < stable_interval.upper), 'state'] = 1
                
                    # Update dataframe to accelerate
                    tmp_df = tmp_df[tmp_df[self.ts_column] >= ts + pd.Timedelta(seconds=1)]
            
            df.loc[(df['Timestamp'] >= interval.lower) & (df['Timestamp'] < interval.upper), 'type'] = tag
            df.loc[(df['Timestamp'] >= interval.lower) & (df['Timestamp'] < interval.upper), 'index'] = i
            
            if not this_df[(this_df['Timestamp'] >= interval.lower) & (this_df['Timestamp'] < interval.upper)].empty:
                df.loc[(df['Timestamp'] >= interval.lower) & (df['Timestamp'] < interval.upper), 'state'] = 1
        ...
        # display(df)
        # display(df[~df[['anchor_type', 'anchor_state', 'type']].duplicated()])
        # display(df[~df[['anchor_type', 'anchor_index', 'anchor_state', 'type', 'index']].duplicated()])
        
        this_df = df[~df[['anchor_type', 'anchor_index', 'anchor_state', 'type', 'index']].duplicated()].copy()
        for anchor_tag, tag in self.corr_data.keys():
            # print(anchor_tag, tag)
            # display(this_df[np.in1d(this_df['anchor_type'], anchor_tag) & np.in1d(this_df['type'], tag)])
            # print(this_df.loc[np.in1d(this_df['anchor_type'], anchor_tag) & np.in1d(this_df['type'], tag), 'anchor_state'].to_list())
            # print(this_df.loc[np.in1d(this_df['anchor_type'], anchor_tag) & np.in1d(this_df['type'], tag), 'state'].to_list())
            
            self.corr_data[(anchor_tag, tag)][0] = [*self.corr_data[(anchor_tag, tag)][0], *this_df.loc[np.in1d(this_df['anchor_type'], anchor_tag) & np.in1d(this_df['type'], tag), 'anchor_state'].to_list()]
            self.corr_data[(anchor_tag, tag)][1] = [*self.corr_data[(anchor_tag, tag)][1], *this_df.loc[np.in1d(this_df['anchor_type'], anchor_tag) & np.in1d(this_df['type'], tag), 'state'].to_list()]
    ...
    def estimate_probability(self):
        ...
        # stable_prob = self.sr_prob_models['Stable']
        # self.dr_prob_models[('Stable', 'Stable')] = (stable_prob, stable_prob)
    ...
    def alter_corr_coef(self, mode='zero', rho_force=None):
        ...
        load_path = os.path.join('.', sr_model_name, dr_model_name, dirc_mets, 'models')
